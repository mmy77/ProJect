...
class MyDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        for i in range(index, index+16):
            ...
            #print(fn)
            img = self.loader(fn)           
            width = img.size[0] #640
            height= img.size[1] #480
            ratio_h = 160/height
            ratio_w = 160/width
           ...
           # print(width,height)
            ...
            if ratio_h>ratio_w: #width>height
                ratio =ratio_h
                img = img.resize((int(width*ratio),160),Image.ANTIALIAS)
                img_data = np.array(img)
                #print("###",img_data.shape)
                #plt.imshow(img_data)
                new_width = int(width*ratio)
                new_height = 160
                img_data = img_data[:160,int(new_width/2)-80:int(new_width/2)+80,:]
            else:#height>width
                ratio =ratio_w
                img = img.resize((160,int(height*ratio)),Image.ANTIALIAS)
                #print(img.size)
                img_data = np.array(img)
                new_height = height*ratio
                img_data = img_data[int(new_height/2)-60:int(new_height/2)+100,:160,:]
               # for x in range(int(width*ratio)):
               #     for y in range(160):
           ...
           # img_data = new_img
            ...
            glist.append(img_g)
            blist.append(img_b)
        global_point = index + 1
        rarray = np.array(rlist)
        blocklist.append(rarray)
        garray = np.array(glist)
        blocklist.append(garray)
        barray = np.array(blist)
        blocklist.append(barray)
...
train_data=MyDataset(txt=root+'croptrainlist.txt', transform=transforms.ToTensor())
#print(img1[0].shape,torch.tensor(img1[0]))
test_data=MyDataset(txt=root+'croptestlist.txt', transform=transforms.ToTensor())
...
def load_data_cross(batch_size=10):
    ...
    for batchpt in range(batch_size):
        ...
        tempdata,templabel,templabel2 = train_data.__getitem__(global_point+random.randint(1,500))
        data.append(tempdata)
...
pretrained_file = './model/p3d_rgb_199.checkpoint.pth.tar'
#predic = premodel.state_dict()
num_classes = 251
model = P3D(Bottleneck, [3, 8, 36, 3],num_classes=num_classes,modality='RGB')
...
for ita in range(5010):
    ...
    if(ita>=2000 and ita%500==0):
        torch.save(model,'./model/rrandom_optimpre199_5cropcross'+str(ita)+'.pkl')

    #print(out.size(),out)
writer.export_scalars_to_json("./log/optimtest.json")
writer.close()
'''
